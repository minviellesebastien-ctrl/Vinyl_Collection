...
let records = JSON.parse(localStorage.getItem(STORAGE_KEY) || "null") || starter;
...
attachLongPress();
window.setTimeout(() => list.classList.add("visible"), 520);
...
if (searchBar) {
  ...
  searchBar.addEventListener('search', searchRecords);
...
const dlgText=document.getElementById('deleteText');
const confirmBtn=document.getElementById('confirmDelete');

function openDeleteDialog(index){
 currentDeleteIndex=index;
 dlgText.textContent=`${records[index].artist} — ${records[index].album}`;
 dlg.showModal();
}
...
function updateCount(){
 const count=document.getElementById('vinylCount');
 if(count){
...
if(count){
 ...
 function anim(t){
  ...
  count.textContent=Math.round(total*e);
  if(p<1)requestAnimationFrame(anim);
  else{
    count.textContent=total;
    count.classList.add('count-bounce');
    count.addEventListener('animationend',()=>count.classList.remove('count-bounce'),{once:true});
  }
 }
...
window.addEventListener("load", refreshBottomBar);
window.addEventListener("resize", refreshBottomBar);
window.addEventListener("orientationchange", refreshBottomBar);

window.addEventListener("pageshow", refreshBottomBar);

window.addEventListener("focus", () => {
  setTimeout(refreshBottomBar, 50);
});
...
if (window.visualViewport) {
  window.visualViewport.addEventListener("resize", refreshBottomBar);
  window.visualViewport.addEventListener("scroll", refreshBottomBar);
}
